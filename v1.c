...
void vely_gen_c_code (vely_gen_ctx *gen_ctx, const char *file_name);
...
num terminal_width();
void out_verbose(num vely_line, const char *format, ...);
...
char *find_keyword(const char *str, const char *find, num has_spaces);
num find_connection(const char *conn);
num add_connection(const char *conn, num dbtype);
...
void process_query (vely_gen_ctx *gen_ctx, const num query_result, const num run_query, const char is_prep, vely_db_parse *vp);
const char *vely_db_vendor(num dbconn);
void trimit(char *var);
void check_vely (char *c);
char *id_from_file (const char *file);
num outargs(char *args, const char *outname, num outtot, num startwith, char pair);
void process_http_header (const char *statement, const char *header, const char *temp_header, num http_header_count, char request);
...
void generate_sql_code (vely_gen_ctx *gen_ctx, char is_prep)
{
    ...
    if (gen_ctx->qry[query_id].returns_tuple == 1)
    {
        ...
        oprintf("vely_select_table (_vv_sql_buf_%s, &_vv_arow_%s, &_vv_nrow_%s, &_vv_ncol_%s, %s%s, &_vv_data_%s, &_vv_dlen_%s, (const char**)&_vv_err_%s, (const char**)&_vv_errm_%s, %d, &_vv_sql_prep_%s, _vv_sql_paramcount_%s, _vv_sql_params_%s, _vv_qry_on_error_%s);\n",
    ...
    else
    {
        ...
        oprintf("vely_execute_SQL (_vv_sql_buf_%s, &_vv_arow_%s, (const char**)&_vv_err_%s, (const char**)&_vv_errm_%s, %lld, 1, %d, &_vv_sql_prep_%s, _vv_sql_paramcount_%s, _vv_sql_params_%s, _vv_qry_on_error_%s);\n",
...
// 'type is VV_DEFSTRING('char *') or VV_DEFNUM or VV_DEFCONSTSTRING or VV_DEFVOIDPTRPTR or VV_DEFCHARPTRPTR
...
//
// IMPORTANT: Note that result assigned to VV_DEFCONSTSTRING must still be Vely allocated (vely_malloc etc) string, only it's const!
//
...
num define_statement (char **statement, num type)
{
    ...
    if (is_def_result == 1)
    {
        ...
        else if (type == VV_DEFCONSTSTRING) oprintf ("VV_DEFINE_CONST_STRING(%s);\n", *statement);
...
//
// Print out help for VELY. Perform splitting/indenting of lines where lines are too long and indented.
...
void tfprintf (FILE *f, const char *format, ...)
{
    ...
    char *oline = NULL;
    num oline_size = 0;
    num tw = terminal_width();

    if (oline == NULL) oline = vely_malloc (oline_size = 2*VV_MAX_CODE_LINE + 1);

    num tot_written = vsnprintf (oline, oline_size - 1, format, args);
    if (tot_written >= oline_size - 1)
    {
        _vely_report_error ("Line being output is too long, exiting");
        exit (1);
    }

    // substitute leading tabs
    const char *tab = "    ";
    num tab_len = strlen (tab);
    num tot_tab = vely_count_substring (oline, "\t", 1); 
    num oline_len = strlen (oline);
    vely_trim (oline, &oline_len);
...
void oprintf (const char *format, ...)
{
    ...
    static num oline_len = 0;
    static num oline_size = 0;
    ...
    // expand if getting long
    if (oline_len >= VV_MAX_CODE_LINE)
    {
            ...
            if (is_verbatim == 0) 
            {
                ...
                if (first_on_line == 1 || usedVELY == 1)
                {
                    ...
                    else if ((newI=recog_statement(line, i, "encode-base64", &mtext, &msize, 0, &vely_is_inline)) != 0)
                    {
                        ...
                        oprintf ("vely_b64_encode ((const char*)(%s), %s, &(%s), %s%s%s);\n", mtext, ilen==NULL?"-1":ilen,to, olen==NULL?"NULL":"&(",olen==NULL?"":olen,olen==NULL?"":")");
                    ...
                    else if ((newI=recog_statement(line, i, "get-sys", &mtext, &msize, 0, &vely_is_inline)) != 0)  
                    {
                        ...
                        // result can be defined
                        define_statement (&to, VV_DEFCONSTSTRING);
                        ...
                        if (env !=NULL) oprintf ("%s=(const char*)(vely_getenv_os(%s));\n", to,env);
                        if (webenv !=NULL) oprintf ("%s=(const char*)(vely_getenv(%s));\n", to,webenv);
                        if (osname !=NULL) oprintf ("%s=(const char*)(vely_os_name());\n", to);
                        if (osver !=NULL) oprintf ("%s=(const char*)(vely_os_version());\n", to);
                    ...
                    else if ((newI=recog_statement(line, i, "get-app", &mtext, &msize, 0, &vely_is_inline)) != 0)
                    {
                        ...
                        else define_statement (&to, VV_DEFCONSTSTRING);

                        num tot_opt = (appdir!=NULL?1:0)+(tracedir!=NULL?1:0)+(filedir!=NULL?1:0)+(apath!=NULL?1:0)+(uploadsize!=NULL?1:0)+(name!=NULL?1:0)+(dbconfname!=NULL?1:0);
                        if (tot_opt != 1)
                        {
                            _vely_report_error( "Exactly one option must be in get-app statement");
                        }

                        if (appdir !=NULL) oprintf ("%s=(const char *)(vely_get_config()->app.home_dir);\n", to); // not alloced
                        if (tracedir !=NULL) oprintf ("%s=(const char *)(vely_get_config()->app.trace_dir);\n", to); // not alloced
                        if (filedir !=NULL) oprintf ("%s=(const char *)(vely_get_config()->app.file_dir);\n", to); // not alloced
                        if (uploadsize !=NULL) oprintf ("%s=vely_get_config()->app.max_upload_size;\n", to); // not alloced
                        if (name !=NULL) oprintf ("%s=(const char*)vely_app_name;\n", to); // not alloced
                        if (dbconfname !=NULL) 
                        {
                            num dbi = find_connection (dbconfname);
                            oprintf ("%s=\"%s\";\n", to, vely_db_vendor(dbi));  // not alloced
                        }
                        if (apath !=NULL) oprintf ("%s=(const char*)vely_app_path;\n", to); // not alloced
                    ...
                    else if ((newI=recog_statement(line, i, "get-req", &mtext, &msize, 0, &vely_is_inline)) != 0)  
                    {
                        ...
                        else if (ctype != NULL || inputval != NULL || inputname != NULL || argval != NULL || cookie != NULL || header != NULL || name != NULL ) define_statement (&to, VV_DEFCONSTSTRING);
                        ...
                        if (tracefile !=NULL) oprintf ("%s=(const char *)(vely_get_config()->trace.fname);\n", to); // not alloced
                        if (numinput !=NULL) oprintf ("%s=vely_get_config()->ctx.req->ip.num_of_input_params;\n", to); // not alloced
                        if (argval !=NULL) oprintf ("%s=(const char *)vely_strdup(vely_get_config()->ctx.req->args.args[%s]);\n", to, argval); //not alloced
                        if (argnum !=NULL) oprintf ("%s=(vely_get_config()->ctx.req->args.num_of_args);\n", to); // not alloced
                        if (inputval !=NULL) oprintf ("%s=(const char *)(vely_get_config()->ctx.req->ip.values[%s]);\n", to, inputval); // not alloced
                        if (inputname !=NULL) oprintf ("%s=(const char *)(vely_get_config()->ctx.req->ip.names[%s]);\n", to, inputname); // not alloced
                        if (ref !=NULL) oprintf ("%s=(const char *)(vely_get_config()->ctx.req->referring_url);\n", to); // not alloced
                        if (numcookie !=NULL) oprintf ("%s=vely_get_config()->ctx.req->num_of_cookies;\n", to); // not alloced
                        if (cookie !=NULL) oprintf ("%s=(const char *)(vely_get_config()->ctx.req->cookies[%s].data);\n", to, cookie); //not alloced
                        if (process_id !=NULL) oprintf ("%s=getpid();\n", to); // not alloced
                        if (eno !=NULL) oprintf ("%s=vely_errno;\n", to); // not alloced
                        if (err !=NULL) oprintf ("%s=(const char*)vely_strdup(strerror(vely_errno));\n", to); // alloced
                        if (header !=NULL) oprintf ("%s=(const char*)vely_getheader(%s);\n", to, header); // not alloced (vely_getenv)
                        if (method !=NULL) oprintf ("%s=vely_get_config()->ctx.req->method;\n", to); // not alloced
                        if (name !=NULL) oprintf ("%s=(const char*)(vely_get_config()->ctx.req->name);\n", to); // not alloced
                        if (ctype !=NULL) oprintf ("%s=(const char*)(vely_getenv(\"CONTENT_TYPE\"));\n", to); // not alloced
...
int main (int argc, char* argv[])
{
    ...
    if (_main == 1)
    {
        ...
        oprintf("const char * vely_app_name=\"%s\";\n", vely_app_name);
        // default application path is /<app name>
        oprintf("const char * vely_app_path=\"%s\";\n", app_path);
        ...
        oprintf("   vely_dbs.conn = calloc (%lld, sizeof(one_db));\n", totconn);
        ...
        oprintf("if (!vv_done_init) _startup();\n");
