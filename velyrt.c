...
num vely_regex(char *look_here, char *find_this, char *replace, char **res, num case_insensitive, num single_match, regex_t **cached)
{
    ...
    {
        reg = &lreg;
        reg_ret = regcomp(reg, find_this, rflags);
...
#define VV_ALLOC_REPLACE(addlen) {res_needed+=((addlen)+1); if (res_allocated <= res_needed) { replace_res_offset = replace_res-*res; *res = (char*)vely_realloc (*res, res_allocated=(res_needed+512)); /* **8** */ replace_res = *res+replace_res_offset; /* **9** */ } }
        ...
        //
        //
        while (*last_look_here != 0 && regexec(reg, last_look_here, sub_num + 1, subexp, 0) == 0) 
        {
            tot_match++; 
        ...
        if (cached == NULL) regfree (reg); // free regcomp() resources, but only if not cached
        return tot_match; 
    ...
    else
    {
        ...
        char regmsg[400];
        regerror(reg_ret, reg, regmsg, sizeof(regmsg)); // get error message and error out
        // free reg *after* regerror
        if (cached != NULL) *cached = NULL; // if tried to cache, invalidate as it is ... invalid
        regfree (reg); // free regcomp resources, but only if not cached
